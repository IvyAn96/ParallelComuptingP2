...
int up_index(int index) {
    int row = 0;
    int col = 0;
    index2position(index, &row, &col);
    row = row - 1;
    return position2index(row,col);
}
...
void *explore(void *input_ptr) {
    ...
    if (is_explorable(up) && up != log->last_index) {
        ...
        pthread_t upThread;
        pthread_create(&upThread, NULL, explore, (void*) new_log);
    ...
    if (is_explorable(down) && down != log->last_index) {
        ...
        pthread_create(&downThread, NULL, explore, (void*)(struct exploration_log *) new_log);
        pthread_mutex_lock(&mutex);
        num_threads++;
        pthread_mutex_unlock(&mutex);
    }
    ...
    if (is_explorable(left) && left != log->last_index) {
        ...
        pthread_create(&leftThread, NULL, explore, (void*)(struct exploration_log *) new_log);
        pthread_mutex_lock(&mutex);
        num_threads++;
        pthread_mutex_unlock(&mutex);
    ...
    if (is_explorable(right) && right != log->last_index) {
        ...
        pthread_create(&rightThread, NULL, explore, (void*)(struct exploration_log *) new_log);
        pthread_mutex_lock(&mutex);
        num_threads++;
        pthread_mutex_unlock(&mutex);
    }
...
int main(int argc, char* argv[]) {
    ...
    if (goal_index >= 0) {
        ...
        printf("Goal location: Index %d, Position (%d, %d)\n",
                goal_index, x, y);
